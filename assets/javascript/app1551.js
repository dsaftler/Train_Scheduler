...
function calcTimeLeft(trainFirst) {
  ...
  let firstTrainTime = today+' '+trainFirst   // date portion + first Time
  console.log("firstTrainTime: "+firstTrainTime);
  let uxFirstTrainTime = moment(firstTrainTime).unix()
  if (now.isBefore(moment(firstTrainTime))){
      //diff between now and firstTrain
    minutesLeft = now.diff(firstTrainTime,'minutes')
  } else if (now.isAfter(moment(firstTrainTime))){
